SELECT C.CustomCIS2 AS 'WorkRequestNumber'
	...
	, CASE WHEN C.NewMeterSerial = '' THEN (SELECT TOP 1 value From CustomCustomerInfo CI WHERE C.CustomerId = CI.CustomerId and CI.[KEY] LIKE 'newsocketapseri%') ELSE C.NewMeterSerial END AS 'NewMeterNumber'
	...
	, CONVERT (datetime, SWITCHOFFSET(Convert(datetimeoffset, J.CreatedDate), DATENAME(Tzoffset,SYSDATETIMEOFFSET()))) AS 'JobCreatedDateTime'
	, CONVERT (datetime, SWITCHOFFSET(Convert(datetimeoffset, J.EndTime), DATENAME(Tzoffset,SYSDATETIMEOFFSET()))) AS 'JobCompletionDate'
	...
	, (SELECT TOP 1 value FROM CustomCustomerInfo CI WHERE C.CustomerId = CI.CustomerId AND CI.[KEY] = 'NewMeterStatus')AS 'MeterStatus'
	...
	, (SELECT TOP 1 value FROM CustomCustomerInfo CI WHERE C.CustomerId = CI.CustomerId AND CI.[KEY] = 'InstallerComments')AS 'InstallerComments'
