SELECT C.customerid
	...
	, CASE WHEN (SELECT TOP 1 value FROM customCustomerInfo CI WHERE C.Customerid = CI.CustomerId AND CI.[Key] = 'LegacyDeviceSerial2') = ''
		...
		ELSE (SELECT TOP 1 value FROM customCustomerInfo CI WHERE C.Customerid = CI.CustomerId AND (CI.[Key] = 'LegacyDeviceSerial1' OR CI.[Key] = 'LegacyDeviceSerial')) END AS 'CT-2-OLD'

	--, (SELECT TOP 1 value FROM customCustomerInfo CI WHERE C.Customerid = CI.CustomerId AND (CI.[Key] = 'LegacyDeviceSerial1' OR CI.[Key] = 'LegacyDeviceSerial') ) AS 'CT-1-OLD'
	...
	, (SELECT TOP 1 value FROM customCustomerInfo CI WHERE C.Customerid = CI.CustomerId AND (CI.[Key] = 'LegacyDeviceRatio2' OR CI.[Key] = 'LegacyDeviceRatio')) AS 'Ratio(as found)'
	...
	, (SELECT TOP 1 value FROM CustomCustomerInfo CI WHERE CI.CustomerId = C.CustomerId AND ( CI.[Key] = 'DeviceRatio1' OR CI.[KEY] = 'DeviceRatio')) AS 'Ratio (as left)'
FROM Customer C
...
OR LEN((SELECT TOP 1 value FROM CustomCustomerInfo CI WHERE CI.CustomerId = C.CustomerId AND CI.[Key] = 'DeviceSerialNumber')) > 0 OR LEN((SELECT TOP 1 value FROM CustomCustomerInfo CI WHERE CI.CustomerId = C.CustomerId AND CI.[Key] = 'LegacyDeviceSerialNumber')) > 0

SELECT *FROM Customer WHERE customerid = '9642212'
SELECT * FROM CustomCustomerInfo WHERE Customerid = '9642212'